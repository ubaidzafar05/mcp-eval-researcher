...
def research(
    ...
    judge_provider: str = typer.Option("ollama", help="Judge provider: ollama."),
...
def doctor(
    ...
    startup_reasons = startup_reason_codes(
        startup_guard_mode=cfg.startup_guard_mode,
        ollama_ready=deps["subsystems"]["ollama"]["ready"],
    )
    ...
    table.add_row("Ollama Endpoint", cfg.ollama_endpoint)
...
def export_pdf(
    run_id: str = typer.Option(..., help="Run id to export to PDF."),
    output: str = typer.Option(None, help="Specific output file path for the PDF."),
) -> None:
    from pathlib import Path
    ...
    try:
        ...
        console.print(f"[bold red]Failed to load run {run_id}: {e}[/bold red]")
        raise typer.Exit(1)

    if not result.final_report:
        console.print(f"[bold red]Run {run_id} does not have a finalized report.[/bold red]")
        raise typer.Exit(1)

    out_dir = Path(output).parent if output else Path(cfg.output_dir)
    ...
    try:
        ...
        if output and Path(output).resolve() != pdf_path.resolve():
            import shutil

            shutil.move(str(pdf_path), str(Path(output).resolve()))
            pdf_path = Path(output).resolve()

        console.print(f"[bold green]Successfully exported PDF to:[/bold green] {pdf_path}")
    except Exception as e:
        console.print(f"[bold red]PDF export failed: {e}[/bold red]")
        raise typer.Exit(1)
