...
from core.metrics import record_empty_extraction, record_fallback_claims
...
- Labeled with a status: "verified", "unverified", or "constrained".
- Include a confidence tag: "high", "medium", or "low".
- Optional reason string if status is not verified.
...
Each claim object has these exact keys:
  ...
  - source_title: string
  - source_url: string
  - status: "verified" | "unverified" | "constrained"
...
def _validate_claims(raw_claims: list[dict[str, Any]], doc_count: int) -> list[ExtractedClaim]:
    ...
    for raw in raw_claims:
        ...
        status = str(raw.get("status", "unverified")).strip().lower()
        ...
        if strength not in {"strong", "moderate", "weak"}:
            strength = "moderate"
        if status not in {"verified", "unverified", "constrained"}:
            status = "unverified"
        ...
        valid.append(ExtractedClaim(
            ...
            strength=strength,
            source_title=str(raw.get("source_title", "")) or "Unknown Source",
            source_url=str(raw.get("source_url", "")) or "unknown",
            status=status,
...
def extract_claims(
    ...
    *,
    ...
    """
    if not docs:
        record_empty_extraction("no_source_documents")
    ...
    try:
        ...
        if not isinstance(raw_claims, list):
            record_empty_extraction("claims_field_not_array")
            return ExtractionResult(
        ...
        if not validated:
            record_empty_extraction("empty_claims")
        return ExtractionResult(
            claims=validated,
            provider_used=provider,
            model_used=model,
        ...
        record_empty_extraction("exception")
        return ExtractionResult(
            error=str(exc)[:200],
            provider_used=provider,
            model_used=model,
...
def build_fallback_extraction(
    ...
    *,
    max_claims: int = 3,
    reason: str = "fallback_claim_from_source",
) -> ExtractionResult:
    claims: list[ExtractedClaim] = []
    for idx, doc in enumerate(docs[: max(1, max_claims)], start=1):
        title = (doc.title or "Untitled source").strip()
        snippet = (doc.snippet or doc.content or "").strip()
        evidence = snippet[:320] if snippet else "No snippet available."
        assertion = f"Claim (constrained): {title} — \"{evidence}\""
        claims.append(
            ExtractedClaim(
                source_id=f"C{idx}",
                topic="fallback",
    ...
    if not claims:
        claims.append(
            ExtractedClaim(
                source_id="C1",
                topic="fallback",
                assertion="Claim (constrained): No sources were available to extract claims.",
                evidence="No sources were available.",
                strength="weak",
    ...
    record_fallback_claims(len(claims))
    return ExtractionResult(
        claims=claims,
        error=None,
