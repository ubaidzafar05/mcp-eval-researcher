...
INVESTIGATIVE_REQUIRED_HEADINGS = (
    "executive summary",
...
ACCEPTED_HEADINGS = REQUIRED_HEADINGS + INVESTIGATIVE_REQUIRED_HEADINGS + (
...
def _required_headings_for_mode(report_structure_mode: str) -> tuple[str, ...]:
    ...
    if report_structure_mode == "investigative":
        return INVESTIGATIVE_REQUIRED_HEADINGS
...
def assess_report_quality(
    ...
    *,
    ...
    if min_required_sections is None:
        if report_structure_mode == "academic_17":
            min_required_sections = (
                14 if depth == "deep"
                else 12 if depth == "balanced"
                else 10
            )
        elif report_structure_mode == "investigative":
            min_required_sections = (
                9 if depth == "deep"
                else 7 if depth == "balanced"
                else 5
            )
        else:
            min_required_sections = 5 if depth == "deep" else 4 if depth == "balanced" else 3
    if insight_density_min is None:
        if report_structure_mode == "academic_17":
            insight_density_min = 8
        elif report_structure_mode == "investigative":
            insight_density_min = 10 if depth == "deep" else 6 if depth == "balanced" else 3
        else:
            insight_density_min = 10 if depth == "deep" else 4 if depth == "balanced" else 2
    ...
    elif report_structure_mode == "decision_brief":
    ...
    if report_structure_mode == "decision_brief":
        ...
        if executive_words < min_exec_words or executive_claim_density > 0.08:
            reasons.append(
                "Executive summary is too thin for investigative depth."
            )
    ...
    register_rows = re.findall(
        r"(?im)^\|\s*\[C\d+\]\s*\|\s*(verified|constrained|withheld|unverified)\s*\|",
        body,
    )
    total_register = len(register_rows)
    constrained_or_unverified = sum(
        1 for status in register_rows if status.lower() in {"constrained", "withheld", "unverified"}
    )
    ctier_proxy_ratio = constrained_or_unverified / max(1, total_register)
    if (
