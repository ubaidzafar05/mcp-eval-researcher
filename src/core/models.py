...
class ClaimRecord(BaseModel):
    claim_id: str
    assertion: str
    status: Literal["verified", "unverified", "constrained", "withheld"] = "constrained"
...
class RunConfig(BaseModel):
    ...
    availability_policy: Literal["must_be_open", "recent_or_open", "unknown_allowed"] = (
        "must_be_open"
    )
    ...
    report_artifact_mode: Literal["narrative_plus_register", "narrative_only", "register_only"] = (
        "narrative_plus_register"
    )
    report_completion_mode: Literal["strict_no_placeholders", "template_fill"] = (
        "strict_no_placeholders"
    )
    tier_policy_mode: Literal["primary_only_strict", "hybrid_strict", "broad"] = (
        "primary_only_strict"
    )
    report_structure_mode: Literal["decision_brief", "academic_17", "investigative"] = (
        "investigative"
    )
    availability_enforcement_scope: Literal["intent_triggered", "always", "never"] = (
        "intent_triggered"
    )
    opportunity_query_detection: Literal["auto", "strict", "off"] = "auto"
    report_surface_mode: Literal[
        "decision_brief_only", "brief_plus_confidence", "full_technical"
    ] = "decision_brief_only"
    ...
    relaxed_quality_mode: bool = False
    ...
    ollama_model_fast: str = "qwen2.5-coder:1.5b"
    ollama_model_primary: str = "qwen3:8b"
    ollama_model_deep: str = "deepseek-r1:14b"
    ollama_model_embed: str = "nomic-embed-text"
    ollama_probe_timeout_seconds: int = 2
    router_mode: Literal["heuristic", "llm", "hybrid"] = "llm"
    router_model: str = "qwen2.5-coder:1.5b"
    router_escalation_threshold: float = 0.70
    router_max_context_deep: int = 6000
    router_allow_deep_parallel: bool = False
    judge_provider: Literal["ollama"] = "ollama"
    judge_json_mode: Literal["repair_retry_fallback", "strict", "heuristic"] = (
    ...
    insufficient_evidence_output: Literal[
        "constrained_actionable", "fail_closed", "soft_fallback"
    ] = "constrained_actionable"
    ...
    dual_use_depth: Literal["dynamic_defensive", "dynamic_balanced", "dynamic_strict"] = (
        "dynamic_defensive"
    )
    min_external_sources: int = 5
    min_unique_providers: int = 2
    min_tier_ab_sources: int = 2
    max_evidence_quote_chars: int = 600
    ...
    llm_request_timeout_seconds_evaluation: int = 150
