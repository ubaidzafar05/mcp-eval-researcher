...
from core.models import Citation, EvalResult, QueryProfile, RunConfig
...
from evals.judges.ollama_judge import judge_with_ollama
...
class DeepEvalNode:
    ...
    def evaluate(
        ...
        *,
        ...
        tier_ab_count = sum(
            1 for c in citations if (c.source_tier or "").upper() in {"A", "B"}
        )
        if (
            not source_ok
        ...
        # Determine judge — Ollama is the sole provider.
        if self.runtime and self.runtime.model_router:
            ...
            try:
                if run_id and getattr(self.runtime, "tracer", None):
                    self.runtime.tracer.event(
                        run_id,
                        "model_route",
                        "Evaluation model selected",
                        payload={
                            "task": "evaluation",
                ...
                with router.use_tier(model_selection.tier):
                    client = self.runtime.get_llm_client(
                        model_selection.provider,
                        request_timeout_seconds=self.config.llm_request_timeout_seconds_evaluation,
                    )
                ...
                # Fallback to direct Ollama API judge
                result = judge_with_ollama(query, report, citations, coverage, self.config)
        else:
            # Direct Ollama judge (no runtime available)
            result = judge_with_ollama(query, report, citations, coverage, self.config)
