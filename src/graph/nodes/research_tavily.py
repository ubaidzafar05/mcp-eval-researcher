...
def _expand_queries(
    ...
    *,
    ...
    if deep:
        queries.extend(
            [
                f"{query} technical mechanism overview {facet_hint}".strip(),
                f"{query} peer reviewed official standards documentation {facet_hint}".strip(),
                f"{query} limitations false positives false negatives counterevidence".strip(),
            ]
        )
    if policy != "standard":
        queries.append(f"{query} defensive mitigation safeguards monitoring".strip())
...
def _peak_refocus_queries(query: str, facets: list[str]) -> list[str]:
    facet_hint = " ".join(facets[:4]).strip()
    return [
        f"{query} site:arxiv.org OR site:nist.gov OR site:ieee.org {facet_hint}".strip(),
        f"{query} site:acm.org OR site:who.int OR site:oecd.org {facet_hint}".strip(),
        f"{query} standards official documentation benchmark report {facet_hint}".strip(),
    ]
...
def create_research_tavily_node(runtime: GraphRuntime):
    # Per-query call budget: MCP timeout + small buffer.  This caps a single
    # Tavily query so the inner ThreadPoolExecutor never waits past this.
    _QUERY_TIMEOUT = max(30, runtime.config.mcp_call_timeout_seconds + 5)

    def _safe_web_call(query: str, k: int) -> list:
        try:
            return runtime.mcp_client.call_web_tool("tavily_search", query, k)
        except Exception as exc:  # noqa: BLE001
            logger.warning(
                "Tavily MCP call failed: %s", str(exc)[:240],
                extra={"tool": "tavily_search"},
            )
            print(f"Tavily MCP call failed: {exc!r}")
            return []
    ...
    def tavily_node(state: ResearchState) -> dict:
        ...
        with ThreadPoolExecutor(max_workers=top_n_queries, thread_name_prefix="tavily") as pool:
            ...
            try:
                ...
                logger.warning("Tavily inner query pool timed out after %ss; using partial results", _QUERY_TIMEOUT)
                for f in futures:
                    if f.done():
                        try:
        ...
        if any(
            ...
            and str((doc.meta or {}).get("fallback_provider", "")).lower() == "tavily"
            and str((doc.meta or {}).get("fallback_reason", "")).lower() == "provider_quota_exhausted"
            for doc in docs
        ):
            provider_alerts.append("provider_quota_exhausted:tavily")
        ...
        # Peak mode second pass: refocus retrieval toward primary A/B sources when first pass is weak.
        if peak_mode and _tier_ab_count(docs) < runtime.config.min_ab_sources:
            ...
            with ThreadPoolExecutor(max_workers=2, thread_name_prefix="tavily_refocus") as pool:
        ...
        return {
            "tavily_docs": docs,
            "tavily_retrieval_stats": aggregate_stats.as_dict(),
