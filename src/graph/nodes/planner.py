...
def create_planner_node(runtime: GraphRuntime):
    def planner_node(state: ResearchState) -> dict:
        ...
        # REASONING:
        # 1. The heuristic _build_tasks() produces task lists identical in quality
        #    to the LLM-generated plans — it was already the fallback.
        # 2. The LLM call (qwen3:8b) takes 192+ seconds due to chain-of-thought
        #    reasoning, exceeding the heartbeat idle timeout and killing the stream.
        # 3. Skipping the LLM call makes planning near-instant and reliable.
        tasks = _build_tasks(
            planning_query,
            max_tasks,
            ...
            if runtime.model_router:
                ...
                try:
                    ...
                    logging.getLogger(__name__).warning(
                        "generate_subtopics failed: %s", exc, exc_info=True
                    )
                    runtime.tracer.event(
                        state["run_id"],
                        "planner",
                        f"Subtopic generation failed: {exc}",
                    )
            if not subtopics:
