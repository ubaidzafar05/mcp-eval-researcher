...
def create_research_firecrawl_node(runtime: GraphRuntime):
    def _safe_web_call(target: str, mode: str) -> list:
        try:
            return runtime.mcp_client.call_web_tool("firecrawl_extract", target, mode)
        except Exception as exc:  # noqa: BLE001
            logger.warning(
                "Firecrawl MCP call failed",
                extra={"tool": "firecrawl_extract", "error": str(exc)[:240]},
            )
            return []
    ...
    def firecrawl_node(state: ResearchState) -> dict:
        ...
        docs = _safe_web_call(target, "extract")
