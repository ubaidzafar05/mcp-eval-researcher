...
def _dedup_add(left: list[str], right: list[str]) -> list[str]:
    """Merge two lists, preserving order but removing duplicates."""
    seen: set[str] = set(left)
    merged = list(left)
    for item in right:
        if item not in seen:
            seen.add(item)
...
class ResearchState(BaseState, total=False):
    ...
    provider_alerts: Annotated[list[str], _dedup_add]
