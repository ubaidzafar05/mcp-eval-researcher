...
from core.metrics import record_report_failure, record_report_success
...
def _minimal_distributed_state(
    ...
    *,
    ...
    parsed_citations = [
        c if isinstance(c, Citation) else Citation.model_validate(c) for c in (citations or [])
    ]
    return {
        "run_id": run_id,
        "query": query,
...
def _route_after_eval(state: ResearchState) -> str:
    ...
    # Gate failed and no retries left — route to HITL.
    # low_confidence is set by eval_gate node when this path is taken.
    return "hitl"
...
def _dispatch_subresearch(state: ResearchState):
    ...
    for item in subtopics:
        ...
        subtopic_query = (
            getattr(item, "sub_query", None) or str(as_dict.get("sub_query", "")).strip()
        )
...
def create_finalize_node(runtime: GraphRuntime):
    def _inject_method_trace_summary(report_body: str, summary: dict[str, object]) -> str:
        ...
        method_lines = [
            ...
            + ", ".join(lane for lane, used in (summary.get("lanes_run") or {}).items() if used),
            ...
            "- Source tier mix: "
            + ", ".join(
                f"{tier}={count}" for tier, count in (summary.get("source_tier_mix") or {}).items()
            ),
    ...
    def finalize_node(state: ResearchState) -> dict:
        ...
        if (
            runtime.config.report_completion_mode == "strict_no_placeholders"
            and not runtime.config.relaxed_quality_mode
        ):
            if (missing_sections or placeholder_hits) and not fail_closed_report:
                failure_reasons: list[str] = []
                if missing_sections:
                    failure_reasons.append("missing_sections:" + ",".join(missing_sections[:4]))
                if placeholder_hits:
                    failure_reasons.append("placeholder_content:" + ",".join(placeholder_hits[:3]))
                metrics = dict(state.get("metrics", {}))
                metrics["quality_failure_buckets"] = list(
                    dict.fromkeys(
                        [
                            *list(metrics.get("quality_failure_buckets", [])),
                            *(["placeholder_content"] if placeholder_hits else []),
                            *(["top_section_directness"] if missing_sections else []),
                        ]
                    )
                )
        ...
        if decision == "abort":
            ...
            status = "completed"
        if status in {"completed", "completed_low_confidence"}:
            record_report_success(status)
        else:
            record_report_failure(status)
    ...
    if distributed:
        if not runtime.config.enable_distributed:
            ...
            return _minimal_distributed_state(
                ...
                logs=[
                    "Distributed mode is disabled; rerun with inline mode or enable distributed profile."
                ],
            )
