...
from core.models import EvalResult, ResearchResult
...
class ResearchRequest(BaseModel):
    ...
    subreport_failure_policy: (
        Literal["continue_constrained", "retry_once", "fail_closed"] | None
    ) = None
    ...
    report_structure_mode: Literal["decision_brief", "academic_17", "investigative"] | None = None
    ...
    report_surface_mode: (
        Literal["decision_brief_only", "brief_plus_confidence", "full_technical"] | None
    ) = None
    ...
    insufficient_evidence_output: (
        Literal["constrained_actionable", "fail_closed", "soft_fallback"] | None
    ) = None
...
def _request_overrides(request: ResearchRequest) -> dict:
    ...
    if request.quality_profile:
        overrides.update(_quality_profile_overrides(request.quality_profile))
...
def _quality_profile_overrides(profile: Literal["strict", "relaxed"]) -> dict[str, object]:
    if profile != "relaxed":
        return {}
    return {
        ...
        "report_style": "full_narrative",
        "relaxed_quality_mode": True,
        "subreport_min_claims": 1,
        "min_claims_deep": 1,
        "min_primary_claims": 0,
        ...
        "require_corroboration_for_tier_c": False,
        "max_ctier_claim_ratio": 0.9,
        "verification_min_sources_per_claim": 1,
        "verified_findings_min": 1,
        "require_verified_findings_in_direct_answer": False,
        "min_primary_verified_findings": 0,
        "top_section_min_verified_claims": 0,
        "insight_density_min": 4,
        "primary_source_policy": "hybrid",
...
def _startup_diagnostics(config) -> dict[str, object]:
    ...
    deps = dependency_health(config)
    ollama_status = deps.get("subsystems", {}).get("ollama", {})
    ollama_ready = None
    if isinstance(ollama_status, dict):
        ollama_ready = bool(ollama_status.get("ready"))
    reason_codes = startup_reason_codes(
...
def _extract_sse_payload(sse_chunk: str) -> dict | None:
    if not sse_chunk:
        return None
...
def _append_two_paragraph_section(
    lines: list[str],
    heading: str,
    paragraph_one: str,
    paragraph_two: str,
    ...
    lines.append(paragraph_one.strip())
    lines.append("")
    lines.append(paragraph_two.strip())
    lines.append("")


def _build_relaxed_fallback_report(query: str, reason: str) -> str:
    clean_query = (query or "").strip()
    title = "Investigative Research Report"
    if clean_query:
        title = f"Investigative Research Report: {clean_query}"
    reason_line = (reason or "The pipeline ended before producing a full report.").strip()
    lines: list[str] = [f"# {title}", ""]
    _append_two_paragraph_section(
        lines,
        "Executive Summary",
        f"This run completed with partial data. {reason_line}",
        "A structured report is returned so the UI remains usable while evidence collection is retried.",
    )
    _append_two_paragraph_section(
        lines,
        "Background and Context",
        f"The inquiry focuses on: {clean_query or 'the requested topic'}.",
        "Contextual framing normally connects historical trends and current conditions; it is thin here because source retrieval was interrupted.",
    )
    _append_two_paragraph_section(
        lines,
        "Key Questions",
        "What are the core claims, stakeholders, and outcomes relevant to the query? What sources carry primary authority?",
        "Where do data gaps exist that could materially change conclusions once evidence is collected?",
    )
    _append_two_paragraph_section(
        lines,
        "Evidence and Findings",
        "No verified findings were produced in this run because retrieval did not finish.",
        "Once sources are available, this section will list claim statements with supporting evidence and citations.",
    )
    _append_two_paragraph_section(
        lines,
        "Deep Analysis",
        "Interpretation is intentionally limited without sufficient evidence to avoid misleading conclusions.",
        "A full run should connect mechanisms, causal pathways, and implications grounded in corroborated sources.",
    )
    _append_two_paragraph_section(
        lines,
        "Conflicting Evidence",
        "No conflicts were analyzed because the evidence pool is incomplete.",
        "When sources are retrieved, conflicting claims will be compared and explained with provenance.",
    )
    _append_two_paragraph_section(
        lines,
        "Case Studies or Examples",
        "No case studies were captured in this run.",
        "A complete run should include representative real-world examples with dates, actors, and outcomes.",
    )
    _append_two_paragraph_section(
        lines,
        "Limitations of Current Knowledge",
        "This output is constrained by missing or incomplete retrieval results.",
        "Re-running with stable providers and longer timeouts is required for robust conclusions.",
    )
    _append_two_paragraph_section(
        lines,
        "Implications",
        "Implications are provisional until evidence is recovered.",
        "Once sources are available, this section will translate findings into practical and policy-relevant impacts.",
    )
    _append_two_paragraph_section(
        lines,
        "Conclusion",
        "This run produced a report shell but not a validated evidence base.",
        "Retrying the run should convert these placeholders into sourced findings and a defensible synthesis.",
    )
    lines.append("## Sources Used")
    lines.append("No sources available in this run.")
    return "\n".join(lines).strip()
    ...
    *,
    ...
    try:
        while True:
            ...
            warn_threshold = max(
                1, int(idle_threshold * max(0.1, min(0.95, warn_before_idle_ratio)))
            )
                ...
                yield {
                    ...
                    "data": {
                        ...
                        "reason_codes": [
                            ...
                            f"max_runtime_stage_{active_stage}",
                        ],
                        "message": (
...
def research(request: ResearchRequest) -> dict:
    ...
    ollama_status = startup.get("ollama_status", {})
    if isinstance(ollama_status, dict) and not ollama_status.get("ready", True):
        if config.startup_guard_mode == "strict":
            raise HTTPException(
                status_code=503,
                detail=f"Ollama backend unavailable: {ollama_status.get('reason', 'unknown')}",
            )
        reason = f"Ollama backend unavailable: {ollama_status.get('reason', 'unknown')}"
        fallback_report = (
            _build_relaxed_fallback_report(request.query, reason)
            if config.relaxed_quality_mode
            else build_fail_closed_report(request.query, reason=reason)
        )
        result = ResearchResult(
            run_id=f"ollama-unavailable-{uuid.uuid4().hex[:8]}",
            query=request.query,
        ...
        payload["execution_mode_fallback_reason"] = reason
        return payload
...
async def research_stream(
    ...
    subtopic_count_default: int | None = None,
    subtopic_count_max: int | None = None,
    subreport_failure_policy: Literal["continue_constrained", "retry_once", "fail_closed"]
    | None = None,
    report_completion_mode: Literal["strict_no_placeholders", "template_fill"] | None = None,
    tier_policy_mode: Literal["primary_only_strict", "hybrid_strict", "broad"] | None = None,
    report_structure_mode: Literal["decision_brief", "academic_17", "investigative"] | None = None,
    availability_enforcement_scope: Literal["intent_triggered", "always", "never"] | None = None,
    opportunity_query_detection: Literal["auto", "strict", "off"] | None = None,
    report_surface_mode: Literal["decision_brief_only", "brief_plus_confidence", "full_technical"]
    | None = None,
    top_section_min_verified_claims: int | None = None,
    insight_density_min: int | None = None,
    report_voice_mode: Literal["analyst", "neutral"] | None = None,
    ...
    async def stream_wrapper():
        ...
        use_relaxed = bool(config.relaxed_quality_mode)
        # Yield start event
        ...
        if isinstance(ollama_status, dict) and not ollama_status.get("ready", True):
            ...
            warn_payload = {
                ...
                "active_stage": "planning",
                "message": reason,
                "reason_codes": ["dependency_unavailable_ollama"],
            }
            yield f"data: {json.dumps(warn_payload)}\n\n"
            fallback_report = (
                _build_relaxed_fallback_report(query, reason)
                if use_relaxed
                else build_fail_closed_report(query, reason=reason)
            )
            final_payload = {
                "type": "status",
                "stage": "final",
                "data": {
                    "result": {
                        "run_id": last_run_id or "unknown-run",
                        "status": "completed",
                        "final_report": fallback_report,
                        "artifacts_path": "",
                        "constrained_reason_codes": ["dependency_unavailable_ollama"],
                        "subtopic_count": 0,
                        "subtopic_success_count": 0,
                        "subtopic_failed_count": 0,
                    }
                },
            }
            yield f"data: {json.dumps(final_payload)}\n\n"
            yield f"data: {json.dumps({'type': 'done', 'final_emitted': True})}\n\n"
            return
        ...
        try:
            ...
            with GraphRuntime.from_config(config) as runtime:
                ...
                async for sse_chunk in event_generator(events):
                    payload = _extract_sse_payload(sse_chunk)
                    if payload:
                        payload_type = payload.get("type")
                        if payload_type == "status" and payload.get("stage") == "final":
                        ...
                        if payload_type == "done":
                            ...
                            if not final_emitted:
                                reason = "Stream ended without a final report payload."
                                fallback_report = (
                                    _build_relaxed_fallback_report(query, reason)
                                    if use_relaxed
                                    else build_fail_closed_report(query, reason=reason)
            ...
            yield f"data: {json.dumps(err_payload)}\n\n"
            if not final_emitted:
                reason = f"Stream error before final report payload: {e}"
                fallback_report = (
                    _build_relaxed_fallback_report(query, reason)
                    if use_relaxed
                    else build_fail_closed_report(query, reason=reason)
...
async def export_pdf(request_body: dict) -> StreamingResponse:
    """Generate a PDF report from a completed research run."""
    from pathlib import Path

    from core.models import Citation, EvalResult
    from core.pdf_export import export_report_to_pdf

    run_id = request_body.get("run_id", "export")
    report = request_body.get("report", "")
    raw_citations = request_body.get("citations", [])
    raw_eval = request_body.get("eval_result")
    ...
    if raw_eval:
        try:
            eval_result = EvalResult(**raw_eval)
        except Exception:  # noqa: BLE001
            pass

    output_dir = Path(load_config({"interactive_hitl": False}).output_dir)
